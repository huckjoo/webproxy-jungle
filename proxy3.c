...
void doit(int connfd)
{
  ...
  if ((cache_index = cache_find(url_store)) != -1)
  {
    ...
    //캐쉬에 저장해 놓은 것들로(캐쉬로) 바로 client로 보낸다.
...
void readerPre(int i)
{
  //readcountmutex는 0이었는데(아무도 안쓰고 있다)
  //readCnt++를 하면 1이 되고 그러면 cache에 접근가능
...
{
  P(&cache.cacheobjs[i].rdcntmutex);
  cache.cacheobjs[i].readCnt--; //내가 cache를 다 썼다.
...
// cache 블록을 10개 만드는데 첫번째로 나오는 빈 cache공간 return
// 10개에서 가장 오래된거 뺀다.
int cache_eviction()
{
...
void cache_LRU(int index)
{
  ...
  for (i = 0; i < index; i++)
  {
    ...
    if (cache.cacheobjs[i].isEmpty == 0 && i != index)
      cache.cacheobjs[i].LRU--; //지금 들어온게 제일 최신이 되기 위해 작업
    writeAfter(i);
  }
  i++; 
...
void cache_uri(char *uri, char *buf)
{
  int i = cache_eviction(); //i는 빈 cache블록

  writePre(i); //p함수 -> 캐쉬에 대한 접근을 풀고 캐쉬에다가 채운다.

  strcpy(cache.cacheobjs[i].cache_obj, buf);
  strcpy(cache.cacheobjs[i].cache_url, uri);
  cache.cacheobjs[i].isEmpty = 0; // 채웠으니까 0으로 바꾼다.
  cache.cacheobjs[i].LRU = LRU_MAGIC_NUMBER; // 9999로 뒤로 미루고
  cache_LRU(i);
